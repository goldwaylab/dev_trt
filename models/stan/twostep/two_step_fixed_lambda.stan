...
transformed parameters {
  ...
  for (s in 1:NS) {
    for (m in 1:M) {
      ...
      alpha1[s,m] = inv_logit(alpha1_logit);
...
model {
  ...
  to_vector(beta_mu) ~ normal(1.1, 1.0);
...
generated quantities {
  ...
  array[NS] real<lower=0> betac_common;
  ...
  for (s in 1:NS) {
    ...
    alpha1_common[s] = inv_logit(alpha1_logit_common);
    ...
    betac_common[s] = exp(betac_log_common);
