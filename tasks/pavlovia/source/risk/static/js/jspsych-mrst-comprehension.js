...
//if (i > -1) {
   jsPsych.pluginAPI.getAudioBuffer(trial.audio[i])
...
// disable to the end of the audio
html += '<input type="submit" id="'+plugin_id_name+'-next" class="'+plugin_id_name+' jspsych-btn"' + (trial.button_label ? ' value="'+trial.button_label + '"': '') + '" disabled="true" style="margin-bottom: 12px;"></input>';

    setTimeout(function(){ document.getElementById('jspsych-survey-multi-choice-next').disabled=false}, trial.view_duration[i]);
}
